...
#' @param ... Additional arguments to be passed to `oa_request()` (e.g., `verbose = F`)
#'
#' @return A list containing the API call results; each element of the outermost list contains data for the works associated with one matched author (some searches return >1 author)
#'
#' @family OpenAlex
#'
#' @examples
...
name_search_oa_request <- function(name, ...) {
  ...
  for (i in 1:length(matching_authors)) {
    ...
    names(author_works)[[i]] <- paste0(matching_authors[[i]]$display_name, ' (', stringi::stri_extract(matching_authors[[i]]$id, regex = '(?<=/)(?i)[A-Z|0-9]+$'), ')')
  }
